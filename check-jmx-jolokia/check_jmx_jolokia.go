...
type jmxJolokiaOpts struct {
	HostName  string  `short:"H" long:"hostname" required:"true" description:"Host name or IP Address"`
	Port      string  `short:"p" long:"post" default:"8778" description:"Port"`
...
func createURL(opts *jmxJolokiaOpts) string {
	if opts.InnerPath == "" {
		return fmt.Sprintf("http://%s:%s/jolokia/read/%s/%s", opts.HostName, opts.Port, opts.MBean, opts.Attribute)
	}
	return fmt.Sprintf("http://%s:%s/jolokia/read/%s/%s/%s", opts.HostName, opts.Port, opts.MBean, opts.Attribute, opts.InnerPath)
}
